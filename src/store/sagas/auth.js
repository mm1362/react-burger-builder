...
export function* authUserSaga(action) {
	...
	if (!action.isSignup) {
		url = 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAgavDvbXyQEHJOspvJSev13FN5XdRV4b0'
	}
...
export function* authCheckStateSaga() {
	...
	if (!token) {
		...
		if (expirationDate <= new Date()) {
			...
			yield put(actions.checkoutAuthTimeout((expirationDate.getTime() - new Date().getTime()) / 1000));
		}
	}
